...
const getBlog = async () => {
    const response = await sendRequest({
        method:'get',
        url:`/v1/blog/${route?.params?.id}`,
    });
    if(response){
        form.value.title = response.data.data.title;
        form.value.author_name = response.data.data.author_name;
        form.value.content = response.data.data.content;
        coverImg.value = response.data.data.image;
    }
}
...
const onSubmit = async() => {
    ...
    if(response?.data){
        toast.success( ' Blog Updated Successfully', {autoclose:1000, 'theme' : 'dark'});
